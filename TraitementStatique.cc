...
ifstream NStream("NodesBDAp.dat", ios::in);
NStream >> NodeB >> NodeD >> NodeAp;
NStream.close();


string filenameTable[6]={"pbcTractionX.out", "pbcTractionY.out", "pbcTractionZ.out", "pbcCisXY.out", "pbcCisXZ.out" , "pbcCisXY.out"};
...
vector<array<size_t, 6> > NodeBResults, NodeDResults, NodeApResults; // Les arrays contiennent Ux Uy Uz Rx Ry Rz
array<size_t, 6> IncNodeBResults, IncNodeDResults, IncNodeApResults; 
...
Inc++;  bool ModeLecture;
	...
	getline(filenameStream, line2, '\n');
	getline(filenameStream, line3, '\n');
	//Line1Stream.clear(); Line2Stream.clear(); Line3Stream.clear();
	Line1Stream.str(line1);
	Line2Stream.str(line2);
	Line3Stream.str(line3);
...
nu12 = -1*( NodeBResults[1] )/( NodeBResults[0] );
nu13 = -1*( NodeBResults[2] )/( NodeBResults[0] );*/

vector<double> V11A, V11B, V12A, V12B, V13A, V13B;
for (size_t i=0; i < NodeBResults.size(); i++){V11A.push_back(NodeBResults[i][3]); V11B.push_back(NodeBResults[i][0]); V12A.push_back(NodeBResults[i][1]); V12B.push_back(NodeBResults[i][0]); V13A.push_back(NodeBResults[i][2]); V13B.push_back(NodeBResults[i][0]);}
...
nu21 = -1*( NodeDResults[0] )/( NodeDResults[1] );
nu23 = -1*( NodeDResults[2] )/( NodeDResults[1] );*/

vector<double> V22A, V22B, V21A, V21B, V23A, V23B;
for (size_t i=0; i < NodeDResults.size(); i++){
V22A.push_back(NodeDResults[i][4]); V22B.push_back(NodeDResults[i][1]); V21A.push_back(NodeDResults[i][1]); V21B.push_back(NodeDResults[i][1]); V23A.push_back(NodeDResults[i][2]); V23B.push_back( NodeDResults[i][1]);}
...
nu31 = -1*( NodeApResults[0] )/( NodeApResults[2] );
nu32 = -1*( NodeApResults[1] )/( NodeApResults[2] );*/

vector<double> V33A, V33B, V31A, V31B, V32A, V32B;
for (size_t i=0; i < NodeDResults.size(); i++){
V33A.push_back(NodeApResults[i][5] ); V33B.push_back( NodeApResults[i][2] ); V31A.push_back(NodeApResults[i][0] ); V31B.push_back(NodeApResults[i][2]); V32A.push_back(NodeApResults[i][1] ); V32B.push_back( NodeApResults[i][2] );
...
cout<<setw(7)<<left<<E11<<nu12<<nu13<<E22<<nu21<<nu23<<E33<<nu31<<nu32<<G12<<G13<<G23<<endl;
